...
class Encoder(nn.Module):
    def __init__(self, 
        ...
        #??????? no use of token in second part
        self.token =  nn.Parameter(torch.randn(16, 1, self.emb_size))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        token = nn.Parameter(torch.randn(batch_size, 1, self.emb_size))  # Shape: (batch_size, 1, emb_size)

        # Concatenate the token with the patch embeddings along dim=1
        out_2 = torch.cat((token, out_1), dim=1)
       
        return out_1, out_2
       

# img = torch.rand(16,3,32,32)
# enc = Encoder(in_channels=3, patch_size=4, emb_size=128)
# out0, _ = enc(img)